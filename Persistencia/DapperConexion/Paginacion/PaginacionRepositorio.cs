    ...
    public class PaginacionRepositorio : IPaginacion
    {
        ...
        public async Task<PaginacionModel> devolverPaginacion(string storeProcedure, int numeroPagina, int cantidadElementos, IDictionary<string, object> parametrosFiltro, string ordenamientoColumna)
        {
            ...
            try
            {
                ...
                foreach (var param in parametrosFiltro) //Caso de venir algun filtro lo agrego uno a uno 
                {
                    parametros.Add("@" + param.Key, param.Value);   //Agrego
                }
                ...
                parametros.Add("@TotalRecords", totalRecords, DbType.Int32, ParameterDirection.Output); // parametro de salida
                parametros.Add("@TotalPaginas", totalPaginas, DbType.Int32, ParameterDirection.Output); // parametro de salida

                var result = await connection.QueryAsync(storeProcedure, parametros, commandType: CommandType.StoredProcedure);
                listaReporte = result.Select(x => (IDictionary<string, object>)x).ToList(); // cambio tipo de dato
